  ...
  return (
    <div className="mt-1 space-y-1">
      ...
      <p className="text-xs text-slate-500">
        Password strength: <span className={`font-medium ${score <= 2 ? 'text-rose-600' : score <= 4 ? 'text-amber-600' : 'text-emerald-600'}`}>{label}</span>
      </p>
      <ul className="text-xs text-slate-500 space-y-0.5 mt-1">
        {[
          ...
          <li key={i} className={ok ? 'text-emerald-600' : 'text-slate-500'}>
            {ok ? '✓' : '○'} {label as string}
          </li>
        ))}
      </ul>
