      ...
      <DashboardLayout>
        <div className="p-8 max-w-4xl">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">NoSQL Injection Demo</h1>
              ...
              <div className="flex flex-wrap gap-2">
                {PAYLOADS.map(p => (
                  <button key={p} onClick={() => setInput(p)}
                    className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-2.5 py-1 rounded font-mono transition-colors">
                    {p}
                  </button>
                ))}
              </div>
              ...
              <ResultPanel blocked={result.blocked ?? false} explanation={result.explanation}>
                {result.query && (
                  <div className="mt-3 space-y-2">
                    ...
                    <pre className="bg-slate-900 rounded p-3 text-xs font-mono text-slate-300 overflow-x-auto">{result.query}</pre>
